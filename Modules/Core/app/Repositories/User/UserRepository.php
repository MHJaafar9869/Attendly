...
class UserRepository extends BaseRepository implements UserRepositoryInterface
{
    ...
    public function register(array $data): mixed
    {
        return DB::transaction(function () use ($data) {
            $data['status_id'] = StatusIDEnum::USER_PENDING;
    ...
    public function forgotPassword(array $credentials): array
    {
        ...
        $method = $status === Password::ResetLinkSent
            ? 'arrayResponseSuccess'
            : 'arrayResponseError';

        return $this->{$method}(__($status));
    }

    public function resetPassword(string $email, string $password, string $token): array // FIXME
    {
